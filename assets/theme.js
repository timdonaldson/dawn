        ...
        function(element) {
          ...
          if (type === 'radio' || type === 'checkbox') {
            if ($element[0].checked) {
              ...
              console.log('activated1: '+currentOption.value);
            ...
            console.log('activated2: '+currentOption.value);
...
theme.Header = (function() {
  ...
  var config = {
    activeClass: 'site-nav--active-dropdown nohover',
...
theme.MobileNav = (function() {
  ...
  function openMobileNav() {
    ...
    cache.$mobileNavContainer.css({
      height: $(window).height() - translateHeaderHeight + 'px'
    });
...
theme.Product = (function() {
  function Product(container) {
    ...
    this.selectors = {
      ...
      savingsPrice: '[data-savings-price]',
  ...
  Product.prototype = _.assignIn({}, Product.prototype, {
    ...
    _initVariants: function() {
      ...
      this.$container.on(
        'variantQtyChange' + this.settings.namespace,
        this._updateQty.bind(this)
      );
    },
    ...
    _liveRegionText: function(variant) {
      ...
      if (variant.compare_at_price > variant.price) {
        ...
        savingsPrice = theme.Currency.formatMoney(
          variant.compare_at_price - variant.price,
          theme.moneyFormat
        );
        
        var percent = variant.compare_at_price - variant.price
        var percent = percent * 100
        var percent = percent / variant.compare_at_price
        
        savingsPercent = Math.round(percent);
      }
      ...
      liveRegionText = liveRegionText
        ...
        .replace('[$]', salePrice)
        .replace('[$]', savingsPrice)
        .replace('[$]', savingsPercent)
        .trim();
    ...
    _updatePrice: function(evt) {
      ...
      var $savingsPrice = $(this.selectors.savingsPrice, $priceContainer);
      var $savingsPercent = $(this.selectors.savingsPercent, $priceContainer);
    ...
    _updateQty: function(evt) {
      var variant = evt.variant;

      // Update the inventory quantity
      // We created a 'variant_inventories' JS array in product.liquid template to get a list of sku inventories
      $.each( variant_inventories, function( i, val ) {
        this_sku = val.split(':')[0]
        this_qty = val.split(':')[1]

        if (variant.sku == this_sku){
          $('#available-stock span').html(this_qty);
        }
      });
...
/* Added by Aisling G @ Shopify Theme Support on December 31, 2018 - REMOVED AGAIN BY TIM IN FAVOUR OF CSS HOVER */

if (window.orientation == undefined){
  //Add Hover to Navigation
  var id = ".site-nav__dropdown, .site-nav--has-dropdown";
  var timer;
 
  $(id).on('mouseleave',(function() {
 
    if(!$('.site-nav--has-dropdown').hasClass('nohover')){
    ...
    $('.site-nav__link').attr("aria-expanded","false");
        var e = $(this);
    e.addClass('site-nav--active-dropdown');
    e.children(":first").attr("aria-expanded","true");
    clearTimeout(timer);
  }));
}
...
if ('ontouchstart' in window) {
  $(".site-nav--has-dropdown .site-nav__link--button").click(function (e) {
    e.preventDefault();
  });
}


//This handles the signup popup cookies for newsletter-popup.liquid section
...
function checkCookie() {
  var user = getCookie("newsletter-popup-viewed");
  if (user != "") {
    ...
    setTimeout(function(){
      ...
      setCookie("newsletter-popup-viewed", 'yes', 1);
    }, 15000);
  }
} 
...
$('#newsletter-popup .newsletter__submit').click(function(){
  setCookie("newsletter-popup-viewed", 'yes', 365);
});

function initCheckCookie(){
  var popup=getCookie("newsletter-popup-viewed");
  if (popup != "") {
    //console.log("Newsletter popup has been viewed");
    //console.log(user);
  }
...
if(sessionStorage.readyforpopup && sessionStorage.readyforpopup === 'ready'){
  ...
  checkCookie('newsletter-popup-viewed');
}

//Fix Navigation for Mobile
if ($(window).width() < 749) {
  site_header = $('.site-header').height()
  announcement_bar = $('.announcement-bar').height()
  mobile_search = $('.mobile-search').height()
  
  $('.announcement-bar').css('top', site_header + 'px');
  $('.mobile-search').css('top', site_header + announcement_bar + 'px');
  $('.main-content').css('padding-top', site_header + announcement_bar + mobile_search + 25 + 'px');
...
$(function() {
  ...
  var searchForms = $('form[action="/search"]').css('position','relative').each(function() {
    ...
    input.attr('autocomplete', 'off').bind('keyup change', function() {
      ...
      if (term.length > 2 && term != $(this).attr('data-old-term')) {
        ...
        resultsList.html('<li><span class="title">Searching...</span></li>');
        $('.search-bar input').css('border-bottom-left-radius', '0px');
        $('.search-bar .btn').css('border-bottom-right-radius', '0px');
  ...
  $('body').bind('click', function(){
    $('.search-results').hide();
    $('.search-bar input').css('border-bottom-left-radius', '3px');
    $('.search-bar .btn').css('border-bottom-right-radius', '3px');
  });
  ...
  $('.product-form__variants').on('change', function() {
    ...
    selected_value = $(this).find(":selected").text().split(" - ")[0].trim();
  ...
  //$(document).on(
   ...
   //   console.log('selecting: '+selected_value);
  //    $('.selector-wrapper select').val(selected_value).trigger('change');      
  //  }
//  );
